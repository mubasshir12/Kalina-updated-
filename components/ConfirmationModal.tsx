...
const ConfirmationModal: React.FC<ConfirmationModalProps> = ({
    ...
    return (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4" aria-modal="true" onClick={onClose}>
            <div className="bg-white dark:bg-dark-sheet rounded-2xl shadow-xl w-full max-w-sm transform transition-all" role="dialog" onClick={(e) => e.stopPropagation()}>
                <div className="p-6">
                ...
                <div className="px-6 py-4 bg-neutral-50 dark:bg-gray-900/50 rounded-b-2xl flex flex-col-reverse sm:flex-row sm:justify-end gap-3">
                    ...
                    <button
                        type="button"
                        className={`w-full sm:w-auto inline-flex justify-center rounded-md border border-transparent shadow-sm px-4 py-2 text-base font-medium text-white focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-neutral-50 dark:focus:ring-offset-dark-sheet ${confirmButtonClasses[confirmButtonVariant]}`}
                        onClick={() => {
                            onConfirm();
                            onClose();
                        }}

...
const SkeletonLoader: React.FC = () => (
    <div className="w-full h-full p-2 space-y-3">
        ...
        <style>{`.shimmer-bg { background-color: #f5f5f5; background-image: linear-gradient(110deg, #f5f5f5 8%, #fafafa 18%, #f5f5f5 33%); background-size: 200% 100%; animation: 1.5s shimmer linear infinite; } .dark .shimmer-bg { background-color: #374151; background-image: linear-gradient(110deg, #374151 8%, #4b5563 18%, #374151 33%); } @keyframes shimmer { to { background-position-x: -200%; } }`}</style>
    </div>
);
                    ...
                    <div className="p-4 flex flex-col">
                        ...
                        <div className="pt-2 flex justify-between items-center text-xs text-neutral-500 dark:text-gray-400 font-mono">
                            ...
                            <div className="flex items-center gap-1">
                                <button onClick={handlePaste} className="p-1.5 rounded-full hover:bg-neutral-100 dark:hover:bg-gray-800" aria-label="Paste text">
                                    <ClipboardPaste className="h-4 w-4" />
                                </button>
                                {inputText && (
                                    <button onClick={() => setInputText('')} className="p-1.5 rounded-full hover:bg-neutral-100 dark:hover:bg-gray-800" aria-label="Clear input text">
                                        <X className="h-4 w-4" />
                                    </button>
                                )}
                            </div>
                    ...
                    <div className="p-4 flex flex-col bg-neutral-50/50 dark:bg-gray-900/40">
                        <div className="relative flex-1 min-h-[100px] flex flex-col">
                            {isLoading ? (
                                ...
                                <p className="whitespace-pre-wrap p-2 text-neutral-800 dark:text-gray-200 text-lg leading-relaxed overflow-y-auto max-h-[40vh] scrollbar-hide flex-1">
                                    {outputText}
                                </p>
                            ) : (
                                <div className="flex-1 flex items-center justify-center text-neutral-400 dark:text-gray-500">
                        ...
                        <div className="pt-2 flex justify-between items-center">
                            ...
                            {!isLoading && outputText && (
                               <button onClick={handleCopy} className="p-2 rounded-full text-neutral-500 dark:text-gray-400 hover:bg-neutral-200 dark:hover:bg-gray-800" aria-label="Copy translation">
